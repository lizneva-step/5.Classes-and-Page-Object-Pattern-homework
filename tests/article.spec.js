...
test.describe("Действия со статьёй", () => {
  ...
  test("Пользователь может создать новую статью", async ({ page }) => {
    ...
    await articlePage.fillArticleTitle(articleData.title);
    await articlePage.fillArticleAbout(articleData.about);
    await articlePage.fillArticleText(articleData.body);
    await articlePage.fillArticleTags(articleData.tags);
